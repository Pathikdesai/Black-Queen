...
- Two aces of a suit beat one: the second wins a second round, since identical cards tie in favour of whoever played first. But that is a reason to like the suit, not to name it trump. Three cards with both aces among them is still a three card holding — two tricks and then nothing, with no length left to cut anybody — so the second copy only counts once the suit is long enough to be a trump suit at all.
...
- Name the same card for both calls when you are long in its suit — six or more at six handed — and both copies are outside your hand. Each copy can bring somebody in, and a long suit is where a partner is worth having and where you can keep control. The drawback is real and is why length is the condition: if both copies sit with one player he joins once and you finish a partner short, which is not a risk worth taking from a short suit. Worth about a point a deal with the length condition, and worth nothing without it.
...
- Holding a called card that has not come down, you already know you are the bidder's partner: he named that card out loud to find one, and you can see your own hand. So do not beat his king with the called ace. Between them those two cards could win two tricks; spent together they win one. That is true whether or not he is holding the other copy, so it does not rest on guessing where that copy is. Hold it and you can still come in on the next lower lead — and in the hands where that chance never arrives, you are left with a card nobody can take off you.
  One exception, and it is the reason the rule works at all: keeping two winners for two tricks is only worth something if the first of them is a trick you actually keep. A king with the other ace still unaccounted for behind it is not that. So when his card is genuinely not safe, nothing else in your hand can protect it, and there are real points on the trick, the called ace covers him after all. Worth about 0.7 points a deal.
...
- Judge that on the players who have not played yet, and on them only. Somebody who has already thrown away cannot cut anything, and a player shown out of trumps cannot cut either. Reading the whole table instead of the seats still to come cost a black queen at a real table: the partner's ace could not be beaten and the one opponent left was visibly out of trumps, but a player who had already acted was out of the suit, so the queen stayed in hand for nothing. Worth about 1.3 points a deal.
...
- When trump is led and trump must be followed, do not feed a partner the fattest trump. That is the ace, and throwing it onto a trick he has already won spends the card that could have taken a whole trick of its own to add ten points the side was collecting anyway. What has to be kept is a card that still *controls* the suit — one nothing outstanding can beat. Below that a trump is just a card: a ten with both aces and both kings still out was never going to win anything, so its five points may as well go into a trick already won. So play the fattest card that is not a controller, the smallest when several are worth the same, the smallest of all if everything left controls the suit, and the only legal card when there is only one — keeping a trump is a preference, never a reason to break following suit.
  Taking the smallest card every time was the first version of this rule, and it measured about 1.2 a deal worse: it protects the ace correctly and then goes on hoarding tens that protect nothing.
...
- That rule covers leads too. She becomes the top spade left the moment the four cards above her are gone, and cashing her then hands twenty points to anyone holding a trump and no spades — cut about two times in three. Where spades are trump none of this applies: she cannot be cut, so the only question is whether a higher spade is still out.
  Worth knowing that this one is in on judgement rather than on the numbers. Measured on fixed deals it costs about 0.2 a deal, behind on six seeds of six, and over 3,900 queens the holder's own side finished with her 60.0% of the time with the rule and 59.9% without — so it changes *when* she is lost more than whether. It is here because a bot throwing twenty points into a cut is wrong to watch and worse to be partnered with, which is a good enough reason at a table full of friends. Recorded so nobody deletes it as dead weight, or re-adds it as a discovery.
